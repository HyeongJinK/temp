...
class UserConfirmButton : UIButton {
    ...
    init(_ pview: UIViewController, frame: CGRect) {
        confirmButtonImage = UIImage(named: "btn_confirm_user", in: Bundle(for: UserConfirmButton.self), compatibleWith: nil)?.stretchableImage(withLeftCapWidth: 8, topCapHeight: 8)
        view = pview
