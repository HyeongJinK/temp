...
public class EgAwsSignInView extends LinearLayout {
     ...
     */
    public EgAwsSignInView(final Context context) {
        this(context, null);
    }
    ...
    private void setUpDivider(final Context context) {
         ...
         */
        if (this.config != null && this.config.getSignInUserPoolsEnabled()) {
            divider = inflate(context, R.layout.eg_aws_custom_horizontal_or_sign_in_divider, null);
        } else {
            divider = inflate(context, R.layout.eg_aws_custom_horizontal_sign_in_divider, null);
        }
        addView(divider);
    }
        ...
        try {
            if (this.config != null) {
                ...
                for (Class<? extends SignInButton> signInButton : signInButtons) {
                    Class<? extends SignInButton> button = replaceButton(signInButton);
                    SignInButton buttonObject = (SignInButton) createDependencyObject(button.getName(),
                            context, button.getCanonicalName());
    ...
    private Class<? extends SignInButton> replaceButton(Class<? extends SignInButton> button) {
        if (FacebookButton.class.isAssignableFrom(button)) {
            return EgAwsFacebookButton.class;
        } else if (GoogleButton.class.isAssignableFrom(button)) {
            return EgAwsGoogleButton.class;
        } else {
            return button;
        }
    }
